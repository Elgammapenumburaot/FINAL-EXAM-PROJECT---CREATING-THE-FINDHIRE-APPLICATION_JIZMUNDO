...
function getJobPosts($user_id) {
    global $pdo;  // Ensure $pdo is being referenced properly
...
// core/models.php
function getHRUsers($user_id) {
    global $pdo;  // Ensure $pdo is being referenced correctly

    // Example of querying the database using $pdo
    $stmt = $pdo->prepare("SELECT * FROM users WHERE role = 'hr' AND id != ?");
    $stmt->execute([$user_id]);
